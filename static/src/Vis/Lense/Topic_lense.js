...
Topic_lense.prototype.update = function(flag){
	...
	//disable transition if panning.
	var is_transition = flag <= 1 ? Topic_lense.enable_transition : false;
	...
	//this condition should be throwed away.
		...
		// if(global_map.getZoom() == 0){
		// 	//zoom level X;
		// 	this.overlay_svg.append("text")
		// 					  .style("fill", "red")
		// 					  .style("stroke", "red")
...
Topic_lense.prototype.update_calculation = function(){
	...
	clusters.forEach(function(entry){
		...
		if(true || pixel_bbox.intersects_by_circle(entry.pixel_bbox)){
...
Topic_lense.cluster_fade_delay_init = 500;
...
Topic_lense.easetype = "cubic-in-out";

Topic_lense.global_clusters = null;

function writeLog(action){

	try{

		var flag = profile.trans;

		var d = new Date();

		$.ajax({
			method: "GET",
			url: "http://"+ip_address+":9000/log",
			data : { flag:flag, action:action, time:d.getTime() }
		})
		.done(function( msg ) {
		});
	}catch(err){
